...
import { ChainId, PersistentGift, PublicKey, TransferAsset } from '../types';
import { ChainAsset } from '../chainRegistry/types';
import { GIFT_STORE } from './constants';

export const backupGifts = (address: string, secret: string, selectedAsset: TransferAsset): void => {
  const gift = {
    timestamp: Date.now(),
    address,
    secret,

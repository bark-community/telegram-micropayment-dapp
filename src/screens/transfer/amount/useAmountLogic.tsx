...
export function useAmountLogic({ prevPage, nextPage, mainButtonText, onAmountChange = () => {} }: AmountPageLogic) {
  ...
  const handleAmountFee = async (selectedAsset: TransferAsset, transferAmount: string) => {
  ...
  const getMaxAmount = async (selectedAsset: TransferAsset) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { fee, formattedDeposit } = await getTransferDetails(selectedAsset as TransferAsset, amount);
      const max = await getMaxAmount(selectedAsset as TransferAsset);
  ...
  const handleChange = (value: string) => {
    ...
    startTransition(async () => {
      const { fee, formattedDeposit } = await getTransferDetails(selectedAsset as TransferAsset, formattedValue);
